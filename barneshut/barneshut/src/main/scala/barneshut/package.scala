...
package object barneshut {
  ...
  case class Fork(
    ...
    val centerX: Float = (List(nw,ne,sw,se).map(_.centerX).min + List(nw,ne,sw,se).map(_.centerX).max) / 2f
    val centerY: Float = (List(nw,ne,sw,se).map(_.centerY).min + List(nw,ne,sw,se).map(_.centerY).max) / 2f
  ...
  extends Quad {
    ...
    val (mass, massX, massY) = (totalMass, bodies.foldLeft(0f)((acc:Float, b:Body) => acc + b.mass * b.x) / totalMass,
      bodies.foldLeft(0f)((acc:Float, b:Body) => acc + b.mass * b.y) / totalMass)
