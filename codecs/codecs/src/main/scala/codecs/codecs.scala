...
import codecs.Json.{Bool, Num, Str}
...
trait EncoderInstances {
  ...
  implicit val stringEncoder: Encoder[String] =
    Encoder.fromFunction(stringVal => Json.Str(stringVal)) // TODO Implement the `Encoder[String]` instance

  /** An encoder for `Boolean` values */
  // TODO Define an implicit value of type `Encoder[Boolean]`
  implicit val booleanEncoder: Encoder[Boolean] =
  Encoder.fromFunction(boolVal => Json.Bool(boolVal))
  /**
...
trait DecoderInstances {
  ...
  implicit val intDecoder: Decoder[Int] = new Decoder[Int] {
    ...
    override def decode(data: Json): Option[Int] = data match {
      case Json.Num(intVal) => if(intVal.isValidInt) Some(intVal.toInt)else None
      case _ => None
    }
  }
  ...
  implicit val stringDecoder: Decoder[String] = new Decoder[String] {
    ...
    override def decode(data: Json): Option[String] = {
      data match {
        case Json.Str(strVal) => Some(strVal)
        case _ => None
      }
    }
  }
  ...
  implicit val booleanDecoder: Decoder[Boolean] = new Decoder[Boolean] {
    ...
    override def decode(data: Json): Option[Boolean] = data match {
      case Json.Bool(boolVal) => Some(boolVal)
      case _ => None
    }
  }
...
//  }
  ...
  (data: Json) => data match { case Json.Arr(arrJson) => Some(arrJson.flatMap(f => decoder.decode(f)))
  }}
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction{
      (data: Json) => data match {
        case Json.Obj(fieldPair) => fieldPair.get(name) match {
          case Some(fieldValue) => decoder.decode(fieldValue)
          case _ => None
        }
      }
    }
...
trait PersonCodecs {
  ...
  implicit val personDecoder: Decoder[Person] = Decoder.field[String]("name").
    zip(Decoder.field[Int]("age")).transform[Person](person => Person(person._1, person._2))
