...
trait Solver extends GameDef {
  ...
  def done(b: Block): Boolean = {
    (b.b1.row == goal.row && b.b1.col == goal.col)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case list =>
        val neightbours: LazyList[(Block, List[Move])] = newNeighborsOnly(neighborsWithHistory(list.head._1,list.head._2),explored)
      (list.head._1, list.head._2) #:: from(list.tail ++ neightbours, explored + list.head._1)
